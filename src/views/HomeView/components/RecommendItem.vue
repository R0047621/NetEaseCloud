<template>
  <div @click=" $router.push(`/SingingListDetails:id=${date.creativeId}`)">
      <div class='w-[31vw] h-[31vw] rounded-[8px] overflow-hidden relative pt-[1vw]'>
         <img v-if="date && date.resources" :src='date.resources[0].uiElement?.image.imageUrl' alt='' class='w-[31vw] h-[31vw] rounded-[8px] relative z-[1]'>
         ...
         <div class='absolute top-0 left-0 pr-[2.4vw] pt-[2vw] justify-end font-[800] text-[#fff] flex items-center w-[31vw] z-[2]'>
            ...
            <span class='font-[800] text-[2.5vw]' v-if="date && date.resources" >{{dataTruncation(date?.resources[0].resourceExtInfo.playCount)}}</span>
         </div>
