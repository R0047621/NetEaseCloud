<template>
  <div class=" bg-[#F4F4F4] dark:bg-[#1a1c23]" :class="valuedata.length !== 0 ? 'h-[100vh]' : '' ">
    <div class="pb-[5vw]">
      <!-- 搜索框 -->
      <div class="w-[91vw] h-[20vw] mx-auto flex justify-between items-center">
        <!-- 返回图标 -->
        <Icon icon="ph:arrow-left-light" width="8vw" class="text-[#000] dark:text-[#e9e9e9]" @click.native="$router.push('/HomeView')"/>
        <!-- 搜索框 -->
        <div class="relative">
          <label for="">
            <input type="text" :placeholder="placeholder" v-model="valuedata" class="dark:border-2 dark:border-[#31333a] dark:bg-[#31333a] dark:text-[#e9e9e9] text-[3vw] pl-[11vw] bg-white opacity-0.2 text-[#9b9b9b] w-[72vw] h-[10vw] border-2 border-[#fff] rounded-[25px] "/>
          </label>
          <Icon icon="tabler:search" color="#6f778f" class="absolute left-[3.68vw] top-[3vw] text-[#000] dark:text-[#e9e9e9]" width="4.3vw" height="4.3vw" @click.native="searchHandler"/>
          <Icon icon="ph:x" color="#6f778f" class="absolute right-[3.68vw] top-[3vw] text-[#000] dark:text-[#e9e9e9]" width="4.3vw" height="4.3vw" v-if="valuedata.length !== 0" @click.native="valuedata = ''"/>
        </div>
      ...
      <div v-if="valuedata.length === 0">
        ...
        <div class="flex justify-around mt-[3vw]">
          <div v-for="(item,index) in menu" :key="item.id" class="dark:border-[#2e2c35] flex w-[25vw] justify-center items-center border-[#E3E5EA]"  :class="{ 'border-r-[0.4vw]': index < 3 }">
        ...
        <div class="mt-[5vw]">
          ...
          <div class="flex px-[3vw]">
            <div v-for="item in like" :key="item.id" class="p-[2vw] mr-[2vw] text-[3.5vw] dark:bg-[#31333a] dark:text-[#e9e9e9] text-[#535c6a] mt-[3vw] bg-[#fff] px-[3vw] rounded-[20px]">{{ item }}</div>
          </div>
        </div>
      ...
      <div v-else >
        <div class="h-[12vw] ml-[3.6vw] flex items-center" v-for="item in search">
          <Icon icon="tabler:search" color="#6f778f" class="text-[#000] dark:text-[#e9e9e9]" width="4.3vw" height="4.3vw"/>
...
<script>
import { playlist,fetchSearchDefault,fetchSearchResult } from '../../repuest'
